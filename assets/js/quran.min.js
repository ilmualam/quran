...
if (qariSel) {
  ...
  Array.from(qariSel.options).forEach(opt => {
    ...
    // loading
    versesEl.innerHTML = `<div class="loading">Memuatkan data surah…</div>`;
    ...
    async function loadAyah(index0, autoplay) {
      ...
      if (autoplay) {
        try {
          ...
          userPaused = true;
          setPlayUI(false);
          toast("Klik ▶️ untuk mula main audio.");
        }
      } else {
        userPaused = true;
        setPlayUI(false);
    ...
    async function togglePlay() {
      ...
      if (audio.paused) {
        ...
        catch { toast("Klik ▶️ untuk mula main audio."); }
      } else {
    ...
    async function copyText(text) {
      try {
        ...
        ta.select();
        try { document.execCommand("copy"); toast("Disalin ✅"); } catch { toast("Copy gagal."); }
        ta.remove();
      }
    }
    ...
    function toggleBookmark(i) {
      ...
      toast(list.has(i) ? "Bookmark ★" : "Buang bookmark");
    }
...
shortenQariLabels();
    ...
    loadSurah().catch(err => {
      console.error(err);
      versesEl.innerHTML = `<div class="loading">❌ Gagal load data. Cuba refresh / semak blocker.<br><small>${String(err?.message || err)}</small></div>`;
  ...
  function ensureEnterpriseToolRow(root) {
    ...
    row.innerHTML = `
      ...
      <div class="act" aria-label="Tindakan">
        <button class="chip" data-action="mode" type="button" title="Tukar mode">🌓</button>
        ...
        <button class="chip" data-action="copy" type="button" title="Copy ayat semasa">⎘</button>
    ...
    if (pp && !pp.textContent.trim()) pp.textContent = "▶️";
  }
  ...
  function ensureFooter(root, sign) {
    ...
    foot.className = "foot";
    foot.innerHTML = `
      <span style="font-size:13px;color:#0c3808;>© 2025 <strong><a href="https://www.ilmualam
      com" target="_blank" rel="noopener">IlmuAlam.com</a></strong> • Quran Reader</span>
    `;
